...
- Headline token/cost figures switched from the ccusage aggregation to the JSONL-native one — and the reconciliation uncovered a misreading in the old accounting: ccusage's daily rollup also covers other agents' session logs (`~/.codex`, the gpt-*/codex-* models ≈ 4.75% of its volume here). The cross-check now compares only models both sides see (`meta.ccusageTotalTokens`), reports the excluded volume as `meta.ccusageOtherAgentsTokens`, and on the same model universe the two totals agree to within ±0.05% on a fresh reconciliation (the diff drifts slightly positive while the ccusage cache ages — see `meta.ccusageReconciledAt`) — not the ~5% gap the old accounting suggested.
